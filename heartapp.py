...
if model is not None:
    ...
    if st.button("Predict Heart Disease"):
        ...
        with col1:
            if prediction == 1:
                st.markdown("<div class='prediction-box disease'>DISEASE RISK</div>",
                            unsafe_allow_html=True)
            else:
                st.markdown("<div class='prediction-box healthy'>HEALTHY</div>",
                            unsafe_allow_html=True)
        ...
        with col3:
            risk = "High Risk" if proba[1] > 0.7 else "Moderate Risk" if proba[1] > 0.4 else "Low Risk"
            st.metric("Risk Level", risk)

        fig = go.Figure(data=[
            go.Bar(name="Healthy", x=["Probability"], y=[proba[0]], marker_color="#2ecc32"),
            go.Bar(name="Disease", x=["Probability"], y=[proba[1]], marker_color="#c62828")
        ])
